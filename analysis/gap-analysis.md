...
**Correction (2026-08-15 post-merge verification):** an earlier draft of this section asserted CJIS mandates prescriptive password parameters (8-char minimum, 3-of-4 complexity, 90-day maximum lifetime, 10-password history) that override 800-63B guidance. None of those four numbers appear in the published v6.1 IA-5 control text. IA-5(f) reads: "Changing or refreshing memorized secret authenticators **annually or when there is evidence of authenticator compromise**." That is the 800-63B-aligned model (event-driven change, no mandatory periodic rotation), not a conflict with it. The modernization to 800-53 Rev 5 (v6.0, Dec 2024) replaced the older CJIS-specific numeric password rules with the standard IA-5 control — there is no surviving CJIS-specific delta on password composition or rotation cadence in v6.1.

This correction matters for the differentiator claim this repo makes: don't assert a CJIS/NIST conflict, or cite a "Section 5.6" that doesn't exist in the current control-ID-based policy structure, without re-verifying against the current text. The prior draft's own "Why this matters" framing (CJIS overrides 800-63B) was internally consistent but built on unverified parameter values.

**What remains genuinely CJIS-specific for IA-5** is the same authenticator-strength expectations FedRAMP already carries — there's no distinct CJIS numeric floor to engineer against. The real IA-2/IA-5 delta for CJIS is the AAL2 Advanced Authentication requirement (see IA-2 above), not password composition.

#### Implementation Guidance

1. **Configure the identity provider to org-defined, 800-63B-aligned password parameters.** Long passphrases over short-complex passwords, breach-corpus screening (per the policy's IA-5 discussion), and annual or compromise-triggered changes. There is no separate CJIS numeric floor to layer on top.
2. **Verify SSO/federated IdP compliance.** If CJI access flows through SSO, confirm the upstream IdP's password policy is documented and consistent with the organization's chosen parameters — CJIS does not impose different values for CJI vs. non-CJI systems on this control.
3. **Don't over-build for a delta that doesn't exist.** If a prior implementation enforced 90-day rotation or 8-char/3-of-4 complexity specifically to "meet CJIS," that's stricter than the policy requires — fine to keep for other reasons, but don't cite it as a CJIS mandate in audit evidence.

#### Evidence Required

An auditor will expect to see:

- **Password policy configuration** from the IdP or directory service, documented and consistent with 800-63B guidance
- **Change-triggering event documentation** — annual refresh, or immediate change on evidence of compromise (IA-5(f))
- **Documentation mapping** the organization's chosen parameters to implemented configuration

#### Key Considerations

- **MFA does not replace password policy.** Even with AAL2 MFA (IA-2), authenticator management (IA-5) is still a distinct control. Satisfying one does not exempt the other.
- **Don't cite a CJIS/NIST conflict that isn't there.** The modernized (v6.0+) policy tracks NIST 800-63B guidance rather than conflicting with it. If earlier documentation or training material asserts otherwise, it's citing a pre-modernization (v5.x) requirement that no longer applies.
...
- **Relationship to AU-6 and AC-2 feedback loop.** AU-6 weekly reviews should flag any CJI access by accounts that are not on the current AC-2 authorized roster. Conversely, AC-2 periodic reviews (annual floor per AC-2(j) — see AC-2 section for the corrected cadence) should use AU-6 data (last access date, access frequency) to inform need-to-know determinations. If an account has gone extended periods without accessing CJI, the reviewer should question whether continued access is justified.
...
**Correction (2026-08-15 post-merge verification):** an earlier draft of this section asserted CJIS requires quarterly (90-day) account reviews. That claim does not hold up against the published text. AC-2(j) in CJIS v6.1 reads: "Review accounts for compliance with account management requirements **at least annually**" — the same cadence FedRAMP High's own ac-02_odp.10 typically uses. There is no CJIS-specific numeric review-frequency delta for AC-2. What remains genuinely CJIS-specific is scope and granularity, not cadence:

- **Need-to-know validation.** Each CJI access authorization must be tied to a specific, documented need-to-know. Generic justifications ("user needs system access for their job") are insufficient. The need-to-know must reference the individual's role, the specific CJI data sets they require, and the business function that requires CJI access. This is more granular than FedRAMP's general "valid access authorization" requirement, but it is not a distinct cadence requirement.
- **Immediate revocation.** AC-2(h) reads: "Notify account managers and system/network administrators within: 1. One day when accounts are no longer required; 2. One day when users are terminated or transferred; and 3. One day when system usage or need-to-know changes." CJIS does not extend this timeline beyond what AC-2 already requires, but the consequence of missing it is higher given CJI sensitivity.

**Why this matters:** The real CJIS-specific engineering surface for AC-2 is need-to-know granularity and the speed of the real-time deprovisioning trigger (transfer, termination, prerequisite lapse), not a shorter periodic-review clock. An agency running annual reviews is compliant on cadence; the gap most audits actually find is a real-time trigger that doesn't fire promptly on role change or prerequisite lapse (PS-3 rescreening overdue, PS-6 addendum expired, AT-2 training lapsed).

**Recommended practice, not a policy mandate:** reviewing CJI-authorized accounts more frequently than the annual floor — quarterly is a common agency choice — is defensible given CJI sensitivity, and the roster/prerequisite-chain ideas below are worth engineering regardless of cadence. But it should be presented as a recommendation the agency chooses, not as literal CJIS text.

#### Implementation Guidance

1. **Establish the CJI-authorized user roster.** Maintain a definitive list of all accounts authorized to access CJI, including: individual name, account identifier, role, assigned privilege level, need-to-know justification, date of last PS-3 background check, date of last PS-6 Security Addendum execution, and date of last AT-2 CJIS security awareness training. This roster is the single source of truth for CJI access authorization and the primary input for the review cycle the agency selects (annual floor; quarterly is a common stricter choice).
...
2. **Build the access review workflow.** Create a structured review process on the agency-selected cadence (annual floor per AC-2(j); document the chosen cadence in policy):
...
3. **Integrate real-time revocation triggers.** Connect CJI access revocation to HR and personnel processes so that access is revoked within one day of (AC-2.h):
...
4. **Cross-reference with AU-6 audit data.** Use CJI access log data from the weekly AU-6 reviews to inform the periodic AC-2 review. Specifically: identify accounts with no recent CJI access (candidates for access removal under least privilege), identify accounts with unusual access patterns (potential indicator of compromised credentials or misuse), and verify that all CJI access events in the audit logs were performed by accounts on the authorized roster.
...
- **Access review reports** with reviewer certifications for each CJI-authorized account, completed on the agency's documented cadence (annual floor per AC-2(j))
...
- **Access revocation records** showing accounts removed during or between reviews, with dates and reasons
- **Evidence of prompt revocation** for personnel separations, role changes, or prerequisite expirations (within one day per AC-2.h)
- **Access review policy** documenting the chosen cadence, reviewer assignment, certification requirements, and revocation procedures
...
- **Periodic review is the backstop, not the primary control.** The real-time revocation process (integrated with HR, provisioning/deprovisioning workflows) is the primary control. Periodic reviews catch what the real-time process missed: role changes that were not communicated, contractor engagements that expired without notification, training that lapsed without triggering deprovisioning. If the periodic review is catching a high volume of stale accounts, the real-time process needs improvement.
- **Prerequisite chain creates cascading revocations.** If a user's PS-3 background check expires and rescreening is not completed in time, their CJI access must be suspended regardless of their role or need-to-know. This means the review must check not just "does this person still need access?" but also "are all their prerequisites still current?" A lapsed training certificate (AT-2) or overdue background check (PS-3) is an access revocation trigger, even if the person's job function has not changed.
...
- **Automation reduces review burden.** Manual reviews for a large user population (hundreds of officers across multiple agencies) become unsustainable regardless of cadence. Automate the report generation, prerequisite compliance checking, and last-access-date calculation. Reserve human judgment for the certification decision itself: "Does this person still need this level of CJI access?"
- **Relationship to PS-3 and PS-6.** AC-2 reviews enforce the ongoing validity of PS-3 (personnel screening) and PS-6 (access agreements). A person who was properly screened and signed the Security Addendum at onboarding may fall out of compliance if rescreening is overdue or the addendum terms have changed. AC-2 is the recurring checkpoint that verifies the entire access authorization chain remains intact.
- **Relationship to AU-6.** AC-2 and AU-6 form a feedback loop. AU-6 weekly reviews detect who is actually accessing CJI. AC-2 periodic reviews verify who is authorized to access CJI. Discrepancies between the two (access by unauthorized accounts, or authorized accounts with no access activity) are findings that require investigation. Implementing these controls together produces stronger assurance than either control alone.
...
**Correction (2026-08-15 post-merge verification):** an earlier draft of this section asserted a CJIS-specific "6 months to initial training, biennial refresher thereafter" cadence. Neither figure appears in the published v6.1 text. AT-2(a)(1) reads: "Provide security and privacy literacy training to system users... as part of initial training for new users prior to accessing CJI **and annually thereafter**" — the same cadence as the FedRAMP High baseline described above. There is no separate "CJIS Security Awareness Training" track with its own timing in the modernized (800-53 Rev 5-aligned) policy; AT-2 is AT-2. The genuinely CJIS-specific surface is content, not cadence:

- **CJI-specific content is expected, not just generic security hygiene.** CJIS-authorized personnel need training content that covers CJI handling and dissemination rules (what constitutes CJI, who it may be shared with, and dissemination restrictions on criminal history record information), the Security Addendum obligations they signed under PS-6, the CJIS incident reporting chain (state CSA-designated recipient per IR-6), and sanctions for policy violations (which can be federal or state criminal offenses, not just administrative). This is elaboration/recommended content scope, not a verbatim CJIS Security Policy citation — verify against the current CJIS Security Awareness Training curriculum your state CSA distributes.
- **Training tied to the access authorization chain.** Tracking training status per individual with CJI access is good practice regardless of exact cadence: expired or never-completed training is a reasonable basis for access suspension, alongside PS-3 (background check) and PS-6 (Security Addendum).

**Why this matters:** A CSP operating under FedRAMP High likely already has annual security awareness training, but the content is almost certainly generic: phishing, password hygiene, reporting suspicious activity. The actual CJIS-specific work is making sure that annual training's *content* covers CJI handling rules, Security Addendum obligations, CJIS-specific incident reporting, and sanctions — not standing up a separate biennial program. Cadence is not the gap; content is.

#### Implementation Guidance

1. **Develop or acquire CJIS-specific training content.** Create training content covering the required topic areas (CJI handling, Security Addendum, incident reporting, sanctions). Content sources include: state CSA-provided training materials (some states publish or distribute CJIS training content), commercial CJIS training vendors, or in-house developed content based on the CJIS Security Policy v6.1 Awareness and Training (AT) family. Ensure content is updated when CJIS policy changes (content must reflect current v6.x requirements, not v5.x).
2. **Track training status per individual with CJI access.** Maintain a training tracking system that records, for each CJI-authorized user: date of initial training completion, date of most recent annual refresh, training content version, and completion status. Integrate the tracking system with the CJI-authorized user roster (from AC-2 implementation guidance) so training status is part of the periodic access review.
3. **Complete initial training prior to CJI access.** AT-2(a)(1) requires initial training as part of onboarding, before access, and annually thereafter — the same cadence as the FedRAMP baseline. Build the training-completion check into the same provisioning workflow that grants CJI access, rather than a separate downstream gate.

   **AWS Implementation:** For personnel who manage CJI infrastructure in AWS, tag IAM users or Identity Center users with training completion metadata (e.g., `cjis-training-completed: 2026-04-15`). Use Lambda scheduled functions to query the tags and flag users approaching the annual refresh deadline. For access suspension automation, Lambda can remove users from the CJI access permission set in IAM Identity Center.

4. **Document content coverage mapping.** Create a mapping showing how the training content addresses each CJIS-specific topic area (CJI handling, Security Addendum, incident reporting, sanctions). This is audit evidence: the auditor will want to verify that the training actually covers these topics, not just that training was completed.
5. **Integrate with AC-2 periodic reviews.** The AC-2 review process (annual floor per AC-2(j)) should include verification of training currency. Any CJI-authorized user with expired or overdue training should be flagged for access suspension during the review.
...
- **CJIS-relevant training curriculum or content** covering CJI handling, Security Addendum obligations, incident reporting, and sanctions
- **Content coverage mapping** showing how the training addresses each CJIS-specific topic
- **Training completion records** per individual with CJI access, including initial completion date and annual refresh dates
- **Evidence of initial training prior to CJI access** and annual refresh thereafter (AT-2(a)(1))
- **Training tracking system configuration** showing deadline enforcement and notification workflows
- **Access suspension records** for personnel whose training lapsed and whose CJI access was suspended as a consequence
- **Training content update records** showing content is refreshed when CJIS policy changes (e.g., v6.0 to v6.1 transition)

#### Key Considerations

- **Cadence matches baseline; content is the real delta.** AT-2's frequency (initial, then annual) is identical between CJIS v6.1 and FedRAMP High. Don't build separate tracking infrastructure for a distinct CJIS training cycle — one annual cycle, with CJIS-specific content included, satisfies both.
...
- **Integration with the prerequisite chain.** CJIS access authorization depends on prerequisites being satisfied: PS-3 (background check), PS-6 (Security Addendum), IA-2 (Advanced Authentication enrolled), and AT-2 (training current). During AC-2 periodic reviews, verify all prerequisites; if any is out of compliance, access should be suspended until remediated. AT-2 is the most likely to lapse quietly because training has an expiration date and personnel may not be actively tracking it.
...
- **Agency-side**: The state CSA or a central agency privacy program centrally manages CJIS-specific controls (background check process, Security Addendum administration, periodic access reviews) across the agency's subordinate components.
